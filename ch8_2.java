...
public class ch8_2 {
	public static void main(String[] args) throws Exception {
		...
		BufferedImage nbi = sobel(bi); // 处理图像
		ImageIO.write(nbi, "jpg", new File("output/ch8_2.jpg")); // 输出图像
		long t2 = System.currentTimeMillis(); // 获取程序运行后的时间
		System.out.println("程序运行" + (t2 - t1) + "毫秒"); // 程序结束后进行提示
	}

	public static BufferedImage sobel(BufferedImage bi) {
		...
		for (int y = 1; y < (h - 1); y++) {
			for (int x = 1; x < (w - 1); x++) {
				int r = sobel(getSz(bi, x, y, 1));
				int g = sobel(getSz(bi, x, y, 2));
				int b = sobel(getSz(bi, x, y, 3));
	...
	private  static  int  sobel(int[]  sz)  {
