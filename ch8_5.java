...
public  class  ch8_5  {
        ...
        public  static  void  main(String[]  args)  throws  Exception  {
                ...
                BufferedImage  nbi=sobel(bi,0.5);                                                //处理图像
                ImageIO.write(nbi,  "jpg",  new  File("output/ch8_5.jpg"));                //输出图像
                long  t2=System.currentTimeMillis();                                //获取程序运行后的时间
                System.out.println("程序运行"+(t2-t1)+"毫秒");        //程序结束后进行提示

        }

        public  static  BufferedImage  sobel(BufferedImage  bi,double  k)  {
                ...
                for(int  y=1;y<(h-1);y++)  {
                        for(int  x=1;x<(w-1);x++)  {
                                int  r=sobel(getSz(bi,x,y,1),k);
                                int  g=sobel(getSz(bi,x,y,2),k);
                                int  b=sobel(getSz(bi,x,y,3),k);
